...
def publisher_main():
    ...
    dds.DomainParticipant.register_idl_type(UMAA_SO_LogReport_LogReportType, get_type_name(UMAA_SO_LogReport_LogReportType))
    dds.DomainParticipant.register_idl_type(UMAA_SO_HealthReport_HealthReportType, get_type_name(UMAA_SO_HealthReport_HealthReportType))
    dds.DomainParticipant.register_idl_type(UMAA_SA_GlobalPoseStatus_GlobalPoseReportType, get_type_name(UMAA_SA_GlobalPoseStatus_GlobalPoseReportType))
    dds.DomainParticipant.register_idl_type(UMAA_SA_VelocityStatus_VelocityReportType, get_type_name(UMAA_SA_VelocityStatus_VelocityReportType))
    dds.DomainParticipant.register_idl_type(UMAA_SA_SpeedStatus_SpeedReportType, get_type_name(UMAA_SA_SpeedStatus_SpeedReportType))
    ...
    speed_report_writer = dds.DataWriter(participant.find_datawriter("pub::SpeedReportType"))
    globalpose_report_writer = dds.DataWriter(participant.find_datawriter("pub::GlobalPoseReportType"))
    velocity_report_writer = dds.DataWriter(participant.find_datawriter("pub::VelocityReportType"))
    ...
    while (True):
        ...
        # write Speed
        speed_report_sample.speedThroughWater = speed
        ...
        print(f'Writing Velocity data eastSpeed: {velocity_report_sample.velocity.eastSpeed}')
        print(f'Writing Velocity data northSpeed: {velocity_report_sample.velocity.northSpeed}')
