...
namespace UMAA {
    ...
    namespace SEM {
        ...
        namespace PanTiltSpecs {
            ...
            PanTiltSpecsReportType::PanTiltSpecsReportType (const ::UMAA::Common::Measurement::Position3D_PlatformXYZ& baseOffset_,const ::BasicTypes::Quaternion& baseOrientation_,double panMax_,double panMaxSpeed_,double panMin_,double tiltMax_,double tiltMaxSpeed_,double tiltMin_,const ::UMAA::Common::Measurement::NumericGUID& source_,const ::UMAA::Common::Measurement::DateTime& timeStamp_):
                m_baseOffset_(baseOffset_), 
                m_baseOrientation_(baseOrientation_), 
                m_panMax_(panMax_), 
                m_panMaxSpeed_(panMaxSpeed_), 
                m_panMin_(panMin_), 
                m_tiltMax_(tiltMax_), 
                m_tiltMaxSpeed_(tiltMaxSpeed_), 
                m_tiltMin_(tiltMin_), 
                m_source_(source_), 
                m_timeStamp_(timeStamp_) {
            }

            void PanTiltSpecsReportType::swap(PanTiltSpecsReportType& other_)  noexcept 
            {
                using std::swap;
            ...
            bool PanTiltSpecsReportType::operator == (const PanTiltSpecsReportType& other_) const {
                ...
                if (std::fabs(m_panMax_ - other_.m_panMax_) > std::numeric_limits< double>::epsilon()
                && !(std::fabs(m_panMax_ - other_.m_panMax_) < (std::numeric_limits< double>::min)())) {
                    return false;
                }
                if (std::fabs(m_panMaxSpeed_ - other_.m_panMaxSpeed_) > std::numeric_limits< double>::epsilon()
                && !(std::fabs(m_panMaxSpeed_ - other_.m_panMaxSpeed_) < (std::numeric_limits< double>::min)())) {
                    return false;
                }
                if (std::fabs(m_panMin_ - other_.m_panMin_) > std::numeric_limits< double>::epsilon()
                && !(std::fabs(m_panMin_ - other_.m_panMin_) < (std::numeric_limits< double>::min)())) {
                    return false;
                }
                if (std::fabs(m_tiltMax_ - other_.m_tiltMax_) > std::numeric_limits< double>::epsilon()
                && !(std::fabs(m_tiltMax_ - other_.m_tiltMax_) < (std::numeric_limits< double>::min)())) {
                    return false;
                }
                if (std::fabs(m_tiltMaxSpeed_ - other_.m_tiltMaxSpeed_) > std::numeric_limits< double>::epsilon()
                && !(std::fabs(m_tiltMaxSpeed_ - other_.m_tiltMaxSpeed_) < (std::numeric_limits< double>::min)())) {
                    return false;
                }
                if (std::fabs(m_tiltMin_ - other_.m_tiltMin_) > std::numeric_limits< double>::epsilon()
                && !(std::fabs(m_tiltMin_ - other_.m_tiltMin_) < (std::numeric_limits< double>::min)())) {
                    return false;
                }
