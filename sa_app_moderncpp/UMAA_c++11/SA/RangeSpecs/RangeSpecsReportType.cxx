...
namespace UMAA {
    ...
    namespace SA {
        ...
        namespace RangeSpecs {
            ...
            RangeSpecsReportType::RangeSpecsReportType (bool coordinateTransformation_,double maxHorizontalFOVStopAngle_,double maxRange_,double maxUpdateRate_,double maxVerticalFOVStopAngle_,double minHorizontalFOVStartAngle_,double minRange_,double minUpdateRate_,double minVerticalFOVStartAngle_,const std::string& name_,bool stateActive_,bool stateOff_,bool stateStandby_,bool supportedBzip2_,bool supportedDeflated_,bool supportedLZMA_,bool supportedNoCompression_,const ::UMAA::Common::Measurement::NumericGUID& source_,const ::UMAA::Common::Measurement::DateTime& timeStamp_):
                m_coordinateTransformation_(coordinateTransformation_), 
                m_maxHorizontalFOVStopAngle_(maxHorizontalFOVStopAngle_), 
                m_maxRange_(maxRange_), 
                m_maxUpdateRate_(maxUpdateRate_), 
                m_maxVerticalFOVStopAngle_(maxVerticalFOVStopAngle_), 
                m_minHorizontalFOVStartAngle_(minHorizontalFOVStartAngle_), 
                m_minRange_(minRange_), 
                m_minUpdateRate_(minUpdateRate_), 
                m_minVerticalFOVStartAngle_(minVerticalFOVStartAngle_), 
                m_name_(name_), 
                m_stateActive_(stateActive_), 
                m_stateOff_(stateOff_), 
                m_stateStandby_(stateStandby_), 
                m_supportedBzip2_(supportedBzip2_), 
                m_supportedDeflated_(supportedDeflated_), 
                m_supportedLZMA_(supportedLZMA_), 
                m_supportedNoCompression_(supportedNoCompression_), 
                m_source_(source_), 
                m_timeStamp_(timeStamp_) {
            }

            void RangeSpecsReportType::swap(RangeSpecsReportType& other_)  noexcept 
            {
                using std::swap;
            ...
            bool RangeSpecsReportType::operator == (const RangeSpecsReportType& other_) const {
                ...
                if (std::fabs(m_maxHorizontalFOVStopAngle_ - other_.m_maxHorizontalFOVStopAngle_) > std::numeric_limits< double>::epsilon()
                && !(std::fabs(m_maxHorizontalFOVStopAngle_ - other_.m_maxHorizontalFOVStopAngle_) < (std::numeric_limits< double>::min)())) {
                    return false;
                }
                ...
                if (std::fabs(m_maxUpdateRate_ - other_.m_maxUpdateRate_) > std::numeric_limits< double>::epsilon()
                && !(std::fabs(m_maxUpdateRate_ - other_.m_maxUpdateRate_) < (std::numeric_limits< double>::min)())) {
                    return false;
                }
                if (std::fabs(m_maxVerticalFOVStopAngle_ - other_.m_maxVerticalFOVStopAngle_) > std::numeric_limits< double>::epsilon()
                && !(std::fabs(m_maxVerticalFOVStopAngle_ - other_.m_maxVerticalFOVStopAngle_) < (std::numeric_limits< double>::min)())) {
                    return false;
                }
                if (std::fabs(m_minHorizontalFOVStartAngle_ - other_.m_minHorizontalFOVStartAngle_) > std::numeric_limits< double>::epsilon()
                && !(std::fabs(m_minHorizontalFOVStartAngle_ - other_.m_minHorizontalFOVStartAngle_) < (std::numeric_limits< double>::min)())) {
                    return false;
                }
                ...
                if (std::fabs(m_minUpdateRate_ - other_.m_minUpdateRate_) > std::numeric_limits< double>::epsilon()
                && !(std::fabs(m_minUpdateRate_ - other_.m_minUpdateRate_) < (std::numeric_limits< double>::min)())) {
                    return false;
                }
                if (std::fabs(m_minVerticalFOVStartAngle_ - other_.m_minVerticalFOVStartAngle_) > std::numeric_limits< double>::epsilon()
                && !(std::fabs(m_minVerticalFOVStartAngle_ - other_.m_minVerticalFOVStartAngle_) < (std::numeric_limits< double>::min)())) {
                    return false;
                }
