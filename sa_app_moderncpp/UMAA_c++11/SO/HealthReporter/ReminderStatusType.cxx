...
namespace UMAA {
    ...
    namespace SO {
        ...
        namespace HealthReporter {
            ...
            ReminderStatusType::ReminderStatusType (const std::string& descriptor_,double elapsedTime_,bool reminderConfig_,const ::UMAA::Common::Measurement::NumericGUID& reminderID_,bool reminderStatus_,double serviceInterval_,const ::UMAA::Common::Measurement::NumericGUID& source_,const ::UMAA::Common::Measurement::DateTime& timeStamp_):
                m_descriptor_(descriptor_), 
                m_elapsedTime_(elapsedTime_), 
                m_reminderConfig_(reminderConfig_), 
                m_reminderID_(reminderID_), 
                m_reminderStatus_(reminderStatus_), 
                m_serviceInterval_(serviceInterval_), 
                m_source_(source_), 
                m_timeStamp_(timeStamp_) {
            }

            void ReminderStatusType::swap(ReminderStatusType& other_)  noexcept 
            {
                using std::swap;
            ...
            bool ReminderStatusType::operator == (const ReminderStatusType& other_) const {
                ...
                if (std::fabs(m_elapsedTime_ - other_.m_elapsedTime_) > std::numeric_limits< double>::epsilon()
                && !(std::fabs(m_elapsedTime_ - other_.m_elapsedTime_) < (std::numeric_limits< double>::min)())) {
                    return false;
                }
                ...
                if (std::fabs(m_serviceInterval_ - other_.m_serviceInterval_) > std::numeric_limits< double>::epsilon()
                && !(std::fabs(m_serviceInterval_ - other_.m_serviceInterval_) < (std::numeric_limits< double>::min)())) {
                    return false;
                }
