...
namespace UMAA {
    ...
    namespace Common {
        ...
        namespace Measurement {
            ...
            CovarianceNEDAccelerationAccelerationType::CovarianceNEDAccelerationAccelerationType (double adAd_,double aeAd_,double aeAe_,double anAd_,double anAe_,double anAn_):
                m_adAd_(adAd_), 
                m_aeAd_(aeAd_), 
                m_aeAe_(aeAe_), 
                m_anAd_(anAd_), 
            ...
            void CovarianceNEDAccelerationAccelerationType::swap(CovarianceNEDAccelerationAccelerationType& other_)  noexcept 
            {
                using std::swap;
            ...
            bool CovarianceNEDAccelerationAccelerationType::operator == (const CovarianceNEDAccelerationAccelerationType& other_) const {
                if (std::fabs(m_adAd_ - other_.m_adAd_) > std::numeric_limits< double>::epsilon()
                && !(std::fabs(m_adAd_ - other_.m_adAd_) < (std::numeric_limits< double>::min)())) {
                    return false;
                }
                if (std::fabs(m_aeAd_ - other_.m_aeAd_) > std::numeric_limits< double>::epsilon()
                && !(std::fabs(m_aeAd_ - other_.m_aeAd_) < (std::numeric_limits< double>::min)())) {
                    return false;
                }
                if (std::fabs(m_aeAe_ - other_.m_aeAe_) > std::numeric_limits< double>::epsilon()
                && !(std::fabs(m_aeAe_ - other_.m_aeAe_) < (std::numeric_limits< double>::min)())) {
                    return false;
                }
                if (std::fabs(m_anAd_ - other_.m_anAd_) > std::numeric_limits< double>::epsilon()
                && !(std::fabs(m_anAd_ - other_.m_anAd_) < (std::numeric_limits< double>::min)())) {
                    return false;
                }
                if (std::fabs(m_anAe_ - other_.m_anAe_) > std::numeric_limits< double>::epsilon()
                && !(std::fabs(m_anAe_ - other_.m_anAe_) < (std::numeric_limits< double>::min)())) {
                    return false;
                }
                if (std::fabs(m_anAn_ - other_.m_anAn_) > std::numeric_limits< double>::epsilon()
                && !(std::fabs(m_anAn_ - other_.m_anAn_) < (std::numeric_limits< double>::min)())) {
                    return false;
                }
                return true;
            }
