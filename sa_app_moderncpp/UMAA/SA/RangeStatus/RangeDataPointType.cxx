...
namespace UMAA {
    ...
    namespace SA {
        ...
        namespace RangeStatus {
            ...
            RangeDataPointType::RangeDataPointType (double bearing,bool bearingValidity,const UMAA::Common::Measurement::CovariancePolarType& dataPointCovariance,double inclination,bool inclinationValidity,double range,bool rangeValidity,const UMAA::Common::Measurement::NumericGUID& pointID):
                m_bearing_( bearing ),
                m_bearingValidity_( bearingValidity ),
                m_dataPointCovariance_( dataPointCovariance ),
                m_inclination_( inclination ),
                m_inclinationValidity_( inclinationValidity ),
                m_range_( range ),
                m_rangeValidity_( rangeValidity ),
                m_pointID_( pointID ) {
            }

            #ifdef RTI_CXX11_RVALUE_REFERENCES
            #ifdef RTI_CXX11_NO_IMPLICIT_MOVE_OPERATIONS
            RangeDataPointType::RangeDataPointType(RangeDataPointType&& other_) OMG_NOEXCEPT  :m_bearing_ (std::move(other_.m_bearing_))
            ,
            m_bearingValidity_ (std::move(other_.m_bearingValidity_))
            ,
            m_dataPointCovariance_ (std::move(other_.m_dataPointCovariance_))
            ,
            m_inclination_ (std::move(other_.m_inclination_))
            ,
            m_inclinationValidity_ (std::move(other_.m_inclinationValidity_))
            ,
            m_range_ (std::move(other_.m_range_))
            ,
            m_rangeValidity_ (std::move(other_.m_rangeValidity_))
            ,
            m_pointID_ (std::move(other_.m_pointID_))
            {
            } 

            RangeDataPointType& RangeDataPointType::operator=(RangeDataPointType&&  other_) OMG_NOEXCEPT {
                RangeDataPointType tmp(std::move(other_));
                swap(tmp); 
                return *this;
            }
            #endif
            ...
            void RangeDataPointType::swap(RangeDataPointType& other_)  OMG_NOEXCEPT 
            {
                using std::swap;
            ...
            bool RangeDataPointType::operator == (const RangeDataPointType& other_) const {
                ...
                if (m_inclination_ != other_.m_inclination_) {
                    return false;
                }
