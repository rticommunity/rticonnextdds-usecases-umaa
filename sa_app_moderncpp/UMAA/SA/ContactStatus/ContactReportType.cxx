...
namespace UMAA {
    ...
    namespace SA {
        ...
        namespace ContactStatus {
            ...
            ContactReportType::ContactReportType (const UMAA::Common::Measurement::NumericGUID& source,const UMAA::Common::Measurement::DateTime& timeStamp,const UMAA::Common::Measurement::NumericGUID& contactsSetID):
                m_source_( source ),
                m_timeStamp_( timeStamp ),
                m_contactsSetID_( contactsSetID ) {
            }

            #ifdef RTI_CXX11_RVALUE_REFERENCES
            #ifdef RTI_CXX11_NO_IMPLICIT_MOVE_OPERATIONS
            ContactReportType::ContactReportType(ContactReportType&& other_) OMG_NOEXCEPT  :m_source_ (std::move(other_.m_source_))
            ,
            m_timeStamp_ (std::move(other_.m_timeStamp_))
            ,
            m_contactsSetID_ (std::move(other_.m_contactsSetID_))
            {
            } 

            ContactReportType& ContactReportType::operator=(ContactReportType&&  other_) OMG_NOEXCEPT {
                ContactReportType tmp(std::move(other_));
                swap(tmp); 
                return *this;
            }
            #endif
            ...
            void ContactReportType::swap(ContactReportType& other_)  OMG_NOEXCEPT 
            {
                using std::swap;
            ...
            void ContactReportType_ContactsSet::swap(ContactReportType_ContactsSet& other_)  OMG_NOEXCEPT 
            {
                using std::swap;
