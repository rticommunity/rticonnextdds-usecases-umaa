...
namespace UMAA {
    namespace SA {
        namespace ECEFPoseStatus {
            ...
            static const std::string ECEFPoseReport_TOPIC = "UMAA::SA::ECEFPoseStatus::ECEFPoseReport";

            class NDDSUSERDllExport ECEFPoseReportType {
              public:
                ECEFPoseReportType();

                ECEFPoseReportType(const UMAA::Common::Measurement::Orientation_PlatformECEF& attitude,double xPosition,double yPosition,double zPosition,const UMAA::Common::Measurement::NumericGUID& source,const UMAA::Common::Measurement::DateTime& timeStamp);

                #ifdef RTI_CXX11_RVALUE_REFERENCES
                #ifndef RTI_CXX11_NO_IMPLICIT_MOVE_OPERATIONS
                ECEFPoseReportType (ECEFPoseReportType&&) = default;
                ECEFPoseReportType& operator=(ECEFPoseReportType&&) = default;
                ECEFPoseReportType& operator=(const ECEFPoseReportType&) = default;
                ECEFPoseReportType(const ECEFPoseReportType&) = default;
                #else
                ECEFPoseReportType(ECEFPoseReportType&& other_) OMG_NOEXCEPT;  
                ECEFPoseReportType& operator=(ECEFPoseReportType&&  other_) OMG_NOEXCEPT;
                #endif
                #endif 

                UMAA::Common::Measurement::Orientation_PlatformECEF& attitude() OMG_NOEXCEPT {
                    return m_attitude_;
                }

                const UMAA::Common::Measurement::Orientation_PlatformECEF& attitude() const OMG_NOEXCEPT {
                    return m_attitude_;
                }
                ...
                double& xPosition() OMG_NOEXCEPT {
                    return m_xPosition_;
                }

                const double& xPosition() const OMG_NOEXCEPT {
                    return m_xPosition_;
                }
                ...
                double& yPosition() OMG_NOEXCEPT {
                    return m_yPosition_;
                }

                const double& yPosition() const OMG_NOEXCEPT {
                    return m_yPosition_;
                }
                ...
                double& zPosition() OMG_NOEXCEPT {
                    return m_zPosition_;
                }

                const double& zPosition() const OMG_NOEXCEPT {
                    return m_zPosition_;
                }
                ...
                void swap(ECEFPoseReportType& other_) OMG_NOEXCEPT ;
            ...
            inline void swap(ECEFPoseReportType& a, ECEFPoseReportType& b)  OMG_NOEXCEPT 
            {
                a.swap(b);
            }
