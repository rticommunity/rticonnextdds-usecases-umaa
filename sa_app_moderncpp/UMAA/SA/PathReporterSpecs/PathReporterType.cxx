...
namespace UMAA {
    ...
    namespace SA {
        ...
        namespace PathReporterSpecs {
            ...
            PathReporterType::PathReporterType (double maxDistance,int32_t maxPoints,double maxTgtResolution,double maxTime,double minTgtResolution,const UMAA::Common::MaritimeEnumeration::PathWayEnumType::PathWayEnumType& pathType):
                m_maxDistance_( maxDistance ),
                m_maxPoints_( maxPoints ),
                m_maxTgtResolution_( maxTgtResolution ),
                m_maxTime_( maxTime ),
                m_minTgtResolution_( minTgtResolution ),
                m_pathType_( pathType ) {
            }

            #ifdef RTI_CXX11_RVALUE_REFERENCES
            #ifdef RTI_CXX11_NO_IMPLICIT_MOVE_OPERATIONS
            PathReporterType::PathReporterType(PathReporterType&& other_) OMG_NOEXCEPT  :m_maxDistance_ (std::move(other_.m_maxDistance_))
            ,
            m_maxPoints_ (std::move(other_.m_maxPoints_))
            ,
            m_maxTgtResolution_ (std::move(other_.m_maxTgtResolution_))
            ,
            m_maxTime_ (std::move(other_.m_maxTime_))
            ,
            m_minTgtResolution_ (std::move(other_.m_minTgtResolution_))
            ,
            m_pathType_ (std::move(other_.m_pathType_))
            {
            } 

            PathReporterType& PathReporterType::operator=(PathReporterType&&  other_) OMG_NOEXCEPT {
                PathReporterType tmp(std::move(other_));
                swap(tmp); 
                return *this;
            }
            #endif
            ...
            void PathReporterType::swap(PathReporterType& other_)  OMG_NOEXCEPT 
            {
                using std::swap;
            ...
            bool PathReporterType::operator == (const PathReporterType& other_) const {
                if (m_maxDistance_ != other_.m_maxDistance_) {
                    return false;
                }
                if (m_maxPoints_ != other_.m_maxPoints_) {
                    return false;
                }
                if (m_maxTgtResolution_ != other_.m_maxTgtResolution_) {
                    return false;
                }
                if (m_maxTime_ != other_.m_maxTime_) {
                    return false;
                }
                if (m_minTgtResolution_ != other_.m_minTgtResolution_) {
                    return false;
                }
