...
namespace UMAA {
    ...
    namespace SA {
        ...
        namespace WorldTransform {
            ...
            WorldTransformType::WorldTransformType (double pixelSize_x,double pixelSize_y,double rotation_x,double rotation_y,double upperLeftCoordinate_x,double upperLeftCoordinate_y):
                m_pixelSize_x_( pixelSize_x ),
                m_pixelSize_y_( pixelSize_y ),
                m_rotation_x_( rotation_x ),
                m_rotation_y_( rotation_y ),
                m_upperLeftCoordinate_x_( upperLeftCoordinate_x ),
                m_upperLeftCoordinate_y_( upperLeftCoordinate_y ) {
            }

            #ifdef RTI_CXX11_RVALUE_REFERENCES
            #ifdef RTI_CXX11_NO_IMPLICIT_MOVE_OPERATIONS
            WorldTransformType::WorldTransformType(WorldTransformType&& other_) OMG_NOEXCEPT  :m_pixelSize_x_ (std::move(other_.m_pixelSize_x_))
            ,
            m_pixelSize_y_ (std::move(other_.m_pixelSize_y_))
            ,
            m_rotation_x_ (std::move(other_.m_rotation_x_))
            ,
            m_rotation_y_ (std::move(other_.m_rotation_y_))
            ,
            m_upperLeftCoordinate_x_ (std::move(other_.m_upperLeftCoordinate_x_))
            ,
            m_upperLeftCoordinate_y_ (std::move(other_.m_upperLeftCoordinate_y_))
            {
            } 

            WorldTransformType& WorldTransformType::operator=(WorldTransformType&&  other_) OMG_NOEXCEPT {
                WorldTransformType tmp(std::move(other_));
                swap(tmp); 
                return *this;
            }
            #endif
            ...
            void WorldTransformType::swap(WorldTransformType& other_)  OMG_NOEXCEPT 
            {
                using std::swap;
            ...
            bool WorldTransformType::operator == (const WorldTransformType& other_) const {
                if (m_pixelSize_x_ != other_.m_pixelSize_x_) {
                    return false;
                }
                if (m_pixelSize_y_ != other_.m_pixelSize_y_) {
                    return false;
                }
                if (m_rotation_x_ != other_.m_rotation_x_) {
                    return false;
                }
                if (m_rotation_y_ != other_.m_rotation_y_) {
                    return false;
                }
                if (m_upperLeftCoordinate_x_ != other_.m_upperLeftCoordinate_x_) {
                    return false;
                }
                if (m_upperLeftCoordinate_y_ != other_.m_upperLeftCoordinate_y_) {
                    return false;
                }
                return true;
            }
