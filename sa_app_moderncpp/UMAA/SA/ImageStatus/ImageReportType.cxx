    ...
    namespace SA {
        ...
        namespace ImageStatus {
            ...
            ImageReportType::ImageReportType (const UMAA::Common::Measurement::Position3D_WGS84& cameraPosition,const ::dds::core::optional< std::string >& imageName,const std::string& imageURI,const UMAA::Common::Measurement::DateTime& timestamp,const UMAA::Common::MaritimeEnumeration::ImageFormatEnumType::ImageFormatEnumType& type,const ::dds::core::optional< UMAA::SA::WorldTransform::WorldTransformType >& worldTransform,const UMAA::Common::Measurement::NumericGUID& source,const UMAA::Common::Measurement::DateTime& timeStamp,const UMAA::Common::Measurement::NumericGUID& imageID):
                m_cameraPosition_( cameraPosition ),
                m_imageName_( imageName ),
                m_imageURI_( imageURI ),
                m_timestamp_( timestamp ),
                m_type_( type ),
                m_worldTransform_( worldTransform ),
                m_source_( source ),
                m_timeStamp_( timeStamp ),
                m_imageID_( imageID ) {
            }
            ...
            ImageReportType::ImageReportType(ImageReportType&& other_) OMG_NOEXCEPT  :m_cameraPosition_ (std::move(other_.m_cameraPosition_))
            ,
            m_imageName_ (std::move(other_.m_imageName_))
            ,
            m_imageURI_ (std::move(other_.m_imageURI_))
            ,
            m_timestamp_ (std::move(other_.m_timestamp_))
            ,
            m_type_ (std::move(other_.m_type_))
            ,
            m_worldTransform_ (std::move(other_.m_worldTransform_))
            ,
            ...
            ImageReportType& ImageReportType::operator=(ImageReportType&&  other_) OMG_NOEXCEPT {
                ImageReportType tmp(std::move(other_));
                swap(tmp); 
                return *this;
            }
            #endif
            ...
            void ImageReportType::swap(ImageReportType& other_)  OMG_NOEXCEPT 
            {
                using std::swap;
