...
namespace UMAA {
    ...
    namespace SEM {
        ...
        namespace GPSFixStatus {
            ...
            GPSFixStatusReportType::GPSFixStatusReportType (const ::rti::core::bounded_sequence< double, 11L >& carrierToNoise,int32_t countDown,bool fixValid,bool lowBkupBattery,const UMAA::Common::MaritimeEnumeration::GPSNavigationSolutionEnumType::GPSNavigationSolutionEnumType& navSolution,const UMAA::Common::Measurement::Position3D_WGS84& originPosition,bool pCode,bool timeOut,const UMAA::Common::Measurement::NumericGUID& source,const UMAA::Common::Measurement::DateTime& timeStamp):
                m_carrierToNoise_( carrierToNoise ),
                m_countDown_( countDown ),
                m_fixValid_( fixValid ),
                m_lowBkupBattery_( lowBkupBattery ),
                m_navSolution_( navSolution ),
                m_originPosition_( originPosition ),
                m_pCode_( pCode ),
                m_timeOut_( timeOut ),
                m_source_( source ),
            ...
            GPSFixStatusReportType::GPSFixStatusReportType(GPSFixStatusReportType&& other_) OMG_NOEXCEPT  :m_carrierToNoise_ (std::move(other_.m_carrierToNoise_))
            ,
            m_countDown_ (std::move(other_.m_countDown_))
            ,
            m_fixValid_ (std::move(other_.m_fixValid_))
            ,
            m_lowBkupBattery_ (std::move(other_.m_lowBkupBattery_))
            ,
            m_navSolution_ (std::move(other_.m_navSolution_))
            ,
            m_originPosition_ (std::move(other_.m_originPosition_))
            ,
            m_pCode_ (std::move(other_.m_pCode_))
            ,
            m_timeOut_ (std::move(other_.m_timeOut_))
            ,
            ...
            GPSFixStatusReportType& GPSFixStatusReportType::operator=(GPSFixStatusReportType&&  other_) OMG_NOEXCEPT {
                GPSFixStatusReportType tmp(std::move(other_));
                swap(tmp); 
                return *this;
            }
            #endif
            ...
            void GPSFixStatusReportType::swap(GPSFixStatusReportType& other_)  OMG_NOEXCEPT 
            {
                using std::swap;
