...
namespace UMAA {
    ...
    namespace SEM {
        ...
        namespace ExtendedPayloadStatus {
            ...
            OperationalParamsType::OperationalParamsType (double busCurrent,double busVoltage,double phaseCurrent,double temp):
                m_busCurrent_( busCurrent ),
                m_busVoltage_( busVoltage ),
                m_phaseCurrent_( phaseCurrent ),
                m_temp_( temp ) {
            }

            #ifdef RTI_CXX11_RVALUE_REFERENCES
            #ifdef RTI_CXX11_NO_IMPLICIT_MOVE_OPERATIONS
            OperationalParamsType::OperationalParamsType(OperationalParamsType&& other_) OMG_NOEXCEPT  :m_busCurrent_ (std::move(other_.m_busCurrent_))
            ,
            m_busVoltage_ (std::move(other_.m_busVoltage_))
            ,
            m_phaseCurrent_ (std::move(other_.m_phaseCurrent_))
            ,
            m_temp_ (std::move(other_.m_temp_))
            {
            } 

            OperationalParamsType& OperationalParamsType::operator=(OperationalParamsType&&  other_) OMG_NOEXCEPT {
                OperationalParamsType tmp(std::move(other_));
                swap(tmp); 
                return *this;
            }
            #endif
            ...
            void OperationalParamsType::swap(OperationalParamsType& other_)  OMG_NOEXCEPT 
            {
                using std::swap;
            ...
            bool OperationalParamsType::operator == (const OperationalParamsType& other_) const {
                if (m_busCurrent_ != other_.m_busCurrent_) {
                    return false;
                }
                if (m_busVoltage_ != other_.m_busVoltage_) {
                    return false;
                }
                if (m_phaseCurrent_ != other_.m_phaseCurrent_) {
                    return false;
                }
