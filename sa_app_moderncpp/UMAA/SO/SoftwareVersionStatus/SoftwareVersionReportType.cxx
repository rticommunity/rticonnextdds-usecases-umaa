...
namespace UMAA {
    ...
    namespace SO {
        ...
        namespace SoftwareVersionStatus {
            ...
            SoftwareVersionReportType::SoftwareVersionReportType (const std::string& buildID,const std::string& copyright,const UMAA::Common::Measurement::NumericGUID& name,const std::string& softwareDescription,const std::string& version,const UMAA::Common::Measurement::NumericGUID& source,const UMAA::Common::Measurement::DateTime& timeStamp,const UMAA::Common::Measurement::NumericGUID& softwareID):
                m_buildID_( buildID ),
                m_copyright_( copyright ),
                m_name_( name ),
                m_softwareDescription_( softwareDescription ),
                m_version_( version ),
                m_source_( source ),
                m_timeStamp_( timeStamp ),
                m_softwareID_( softwareID ) {
            }

            #ifdef RTI_CXX11_RVALUE_REFERENCES
            #ifdef RTI_CXX11_NO_IMPLICIT_MOVE_OPERATIONS
            SoftwareVersionReportType::SoftwareVersionReportType(SoftwareVersionReportType&& other_) OMG_NOEXCEPT  :m_buildID_ (std::move(other_.m_buildID_))
            ,
            m_copyright_ (std::move(other_.m_copyright_))
            ,
            m_name_ (std::move(other_.m_name_))
            ,
            m_softwareDescription_ (std::move(other_.m_softwareDescription_))
            ,
            m_version_ (std::move(other_.m_version_))
            ,
            m_source_ (std::move(other_.m_source_))
            ,
            m_timeStamp_ (std::move(other_.m_timeStamp_))
            ,
            m_softwareID_ (std::move(other_.m_softwareID_))
            {
            } 

            SoftwareVersionReportType& SoftwareVersionReportType::operator=(SoftwareVersionReportType&&  other_) OMG_NOEXCEPT {
                SoftwareVersionReportType tmp(std::move(other_));
                swap(tmp); 
                return *this;
            }
            #endif
            ...
            void SoftwareVersionReportType::swap(SoftwareVersionReportType& other_)  OMG_NOEXCEPT 
            {
                using std::swap;
