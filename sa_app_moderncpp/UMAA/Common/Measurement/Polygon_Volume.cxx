...
namespace UMAA {
    ...
    namespace Common {
        ...
        namespace Measurement {
            ...
            Polygon_Volume::Polygon_Volume (const UMAA::Common::Measurement::Altitude_MSL& ceiling,const UMAA::Common::Measurement::Altitude_MSL& floor,const UMAA::Common::Enumeration::LineSegmentEnumType::LineSegmentEnumType& lineKind,const ::rti::core::bounded_sequence< UMAA::Common::Measurement::Position2D, 100L >& referencePoints):
                m_ceiling_( ceiling ),
                m_floor_( floor ),
                m_lineKind_( lineKind ),
                m_referencePoints_( referencePoints ) {
            }

            #ifdef RTI_CXX11_RVALUE_REFERENCES
            #ifdef RTI_CXX11_NO_IMPLICIT_MOVE_OPERATIONS
            Polygon_Volume::Polygon_Volume(Polygon_Volume&& other_) OMG_NOEXCEPT  :m_ceiling_ (std::move(other_.m_ceiling_))
            ,
            m_floor_ (std::move(other_.m_floor_))
            ,
            m_lineKind_ (std::move(other_.m_lineKind_))
            ,
            m_referencePoints_ (std::move(other_.m_referencePoints_))
            {
            } 

            Polygon_Volume& Polygon_Volume::operator=(Polygon_Volume&&  other_) OMG_NOEXCEPT {
                Polygon_Volume tmp(std::move(other_));
                swap(tmp); 
                return *this;
            }
            #endif
            ...
            void Polygon_Volume::swap(Polygon_Volume& other_)  OMG_NOEXCEPT 
            {
                using std::swap;
