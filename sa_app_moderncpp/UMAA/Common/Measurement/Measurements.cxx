...
namespace UMAA {
    ...
    namespace Common {
        ...
        namespace Measurement {
            ...
            DateTime::DateTime (int64_t seconds,int32_t nanoseconds):
                m_seconds_( seconds ),
                m_nanoseconds_( nanoseconds ) {
            }

            #ifdef RTI_CXX11_RVALUE_REFERENCES
            #ifdef RTI_CXX11_NO_IMPLICIT_MOVE_OPERATIONS
            DateTime::DateTime(DateTime&& other_) OMG_NOEXCEPT  :m_seconds_ (std::move(other_.m_seconds_))
            ,
            m_nanoseconds_ (std::move(other_.m_nanoseconds_))
            {
            } 

            DateTime& DateTime::operator=(DateTime&&  other_) OMG_NOEXCEPT {
                DateTime tmp(std::move(other_));
                swap(tmp); 
                return *this;
            }
            #endif
            ...
            void DateTime::swap(DateTime& other_)  OMG_NOEXCEPT 
            {
                using std::swap;
