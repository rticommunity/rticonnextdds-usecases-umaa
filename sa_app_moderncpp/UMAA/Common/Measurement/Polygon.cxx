...
namespace UMAA {
    ...
    namespace Common {
        ...
        namespace Measurement {
            ...
            Polygon::Polygon (const UMAA::Common::Enumeration::LineSegmentEnumType::LineSegmentEnumType& lineKind,const ::rti::core::bounded_sequence< UMAA::Common::Measurement::Position2DTime, 100L >& referencePoint):
                m_lineKind_( lineKind ),
                m_referencePoint_( referencePoint ) {
            }

            #ifdef RTI_CXX11_RVALUE_REFERENCES
            #ifdef RTI_CXX11_NO_IMPLICIT_MOVE_OPERATIONS
            Polygon::Polygon(Polygon&& other_) OMG_NOEXCEPT  :m_lineKind_ (std::move(other_.m_lineKind_))
            ,
            m_referencePoint_ (std::move(other_.m_referencePoint_))
            {
            } 

            Polygon& Polygon::operator=(Polygon&&  other_) OMG_NOEXCEPT {
                Polygon tmp(std::move(other_));
                swap(tmp); 
                return *this;
            }
            #endif
            ...
            void Polygon::swap(Polygon& other_)  OMG_NOEXCEPT 
            {
                using std::swap;
