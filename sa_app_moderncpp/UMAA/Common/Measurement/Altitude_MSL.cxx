...
namespace UMAA {
    ...
    namespace Common {
        ...
        namespace Measurement {
            ...
            Altitude_MSL::Altitude_MSL (double altitude):
                m_altitude_( altitude ) {
            }

            #ifdef RTI_CXX11_RVALUE_REFERENCES
            #ifdef RTI_CXX11_NO_IMPLICIT_MOVE_OPERATIONS
            Altitude_MSL::Altitude_MSL(Altitude_MSL&& other_) OMG_NOEXCEPT  :m_altitude_ (std::move(other_.m_altitude_))
            {
            } 

            Altitude_MSL& Altitude_MSL::operator=(Altitude_MSL&&  other_) OMG_NOEXCEPT {
                Altitude_MSL tmp(std::move(other_));
                swap(tmp); 
                return *this;
            }
            #endif
            ...
            void Altitude_MSL::swap(Altitude_MSL& other_)  OMG_NOEXCEPT 
            {
                using std::swap;
