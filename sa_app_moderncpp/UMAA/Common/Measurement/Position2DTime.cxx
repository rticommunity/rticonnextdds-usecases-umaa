...
namespace UMAA {
    ...
    namespace Common {
        ...
        namespace Measurement {
            ...
            Position2DTime::Position2DTime (const UMAA::Common::Measurement::Position2D& geodeticPosition,const ::dds::core::optional< double >& timeAtPosition):
                m_geodeticPosition_( geodeticPosition ),
                m_timeAtPosition_( timeAtPosition ) {
            }

            #ifdef RTI_CXX11_RVALUE_REFERENCES
            #ifdef RTI_CXX11_NO_IMPLICIT_MOVE_OPERATIONS
            Position2DTime::Position2DTime(Position2DTime&& other_) OMG_NOEXCEPT  :m_geodeticPosition_ (std::move(other_.m_geodeticPosition_))
            ,
            m_timeAtPosition_ (std::move(other_.m_timeAtPosition_))
            {
            } 

            Position2DTime& Position2DTime::operator=(Position2DTime&&  other_) OMG_NOEXCEPT {
                Position2DTime tmp(std::move(other_));
                swap(tmp); 
                return *this;
            }
            #endif
            ...
            void Position2DTime::swap(Position2DTime& other_)  OMG_NOEXCEPT 
            {
                using std::swap;
            ...
            bool Position2DTime::operator == (const Position2DTime& other_) const {
                ...
                if (m_timeAtPosition_ != other_.m_timeAtPosition_) {
                    return false;
                }
                return true;
            }
