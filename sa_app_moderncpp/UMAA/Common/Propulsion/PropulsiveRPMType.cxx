...
namespace UMAA {
    ...
    namespace Common {
        ...
        namespace Propulsion {
            ...
            PropulsiveRPMType::PropulsiveRPMType (double RPM):
                m_RPM_( RPM ) {
            }

            #ifdef RTI_CXX11_RVALUE_REFERENCES
            #ifdef RTI_CXX11_NO_IMPLICIT_MOVE_OPERATIONS
            PropulsiveRPMType::PropulsiveRPMType(PropulsiveRPMType&& other_) OMG_NOEXCEPT  :m_RPM_ (std::move(other_.m_RPM_))
            {
            } 

            PropulsiveRPMType& PropulsiveRPMType::operator=(PropulsiveRPMType&&  other_) OMG_NOEXCEPT {
                PropulsiveRPMType tmp(std::move(other_));
                swap(tmp); 
                return *this;
            }
            #endif
            ...
            void PropulsiveRPMType::swap(PropulsiveRPMType& other_)  OMG_NOEXCEPT 
            {
                using std::swap;
