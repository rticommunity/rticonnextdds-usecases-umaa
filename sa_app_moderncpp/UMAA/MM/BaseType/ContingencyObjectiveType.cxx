    ...
    namespace MM {
        ...
        namespace BaseType {
            ...
            ContingencyObjectiveType::ContingencyObjectiveType (const UMAA::Common::Measurement::Altitude_MSL& altitude,const UMAA::Common::Measurement::Altitude_AGL& altitudeAGL,double altitudeASF,const UMAA::Common::MaritimeEnumeration::ContingencyBehaviorEnumType::ContingencyBehaviorEnumType& behavior,double depth,double DTEDAltitude,const std::string& mode,const UMAA::Common::Measurement::Position2D& position,double radius,double safeAltitude,double safeAltitudeOffset,double speed,double vehicleRunTime):
                ...
                m_behavior_( behavior ),
                m_depth_( depth ),
                m_DTEDAltitude_( DTEDAltitude ),
                m_mode_( mode ),
                m_position_( position ),
                m_radius_( radius ),
                m_safeAltitude_( safeAltitude ),
                m_safeAltitudeOffset_( safeAltitudeOffset ),
                m_speed_( speed ),
                m_vehicleRunTime_( vehicleRunTime ) {
            }

            #ifdef RTI_CXX11_RVALUE_REFERENCES
            #ifdef RTI_CXX11_NO_IMPLICIT_MOVE_OPERATIONS
            ContingencyObjectiveType::ContingencyObjectiveType(ContingencyObjectiveType&& other_) OMG_NOEXCEPT  :m_altitude_ (std::move(other_.m_altitude_))
            ,
            m_altitudeAGL_ (std::move(other_.m_altitudeAGL_))
            ,
            m_altitudeASF_ (std::move(other_.m_altitudeASF_))
            ,
            m_behavior_ (std::move(other_.m_behavior_))
            ,
            m_depth_ (std::move(other_.m_depth_))
            ,
            m_DTEDAltitude_ (std::move(other_.m_DTEDAltitude_))
            ,
            ...
            ,
            m_safeAltitude_ (std::move(other_.m_safeAltitude_))
            ,
            m_safeAltitudeOffset_ (std::move(other_.m_safeAltitudeOffset_))
            ,
            m_speed_ (std::move(other_.m_speed_))
            ,
            m_vehicleRunTime_ (std::move(other_.m_vehicleRunTime_))
            {
            } 

            ContingencyObjectiveType& ContingencyObjectiveType::operator=(ContingencyObjectiveType&&  other_) OMG_NOEXCEPT {
                ContingencyObjectiveType tmp(std::move(other_));
                swap(tmp); 
                return *this;
            }
            #endif
            ...
            void ContingencyObjectiveType::swap(ContingencyObjectiveType& other_)  OMG_NOEXCEPT 
            {
                using std::swap;
            ...
            bool ContingencyObjectiveType::operator == (const ContingencyObjectiveType& other_) const {
                ...
                if (m_DTEDAltitude_ != other_.m_DTEDAltitude_) {
                    return false;
                }
                ...
                if (m_safeAltitude_ != other_.m_safeAltitude_) {
                    return false;
                }
                if (m_safeAltitudeOffset_ != other_.m_safeAltitudeOffset_) {
                    return false;
                }
                if (m_speed_ != other_.m_speed_) {
                    return false;
                }
                if (m_vehicleRunTime_ != other_.m_vehicleRunTime_) {
                    return false;
                }
                return true;
            }
